...
public class StudentMarksAnalyser {
    ...
    String[][] array = new String[read.getData().size() ][20];
    String[] headings = new String[20];
    int tempCount = 3;
    for (int i = 0;i < read.getHeadings().size();i++){
      headings[i] = read.getHeadings().get(i);
        ...
        for (int i = 0; i < read.getData().size() ; i++){ //iterating though a list of data
            ...
            array[i][19] = Integer.toString(read.getData().get(i).getAvgMark());
        }
